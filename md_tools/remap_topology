...
def generate_new_bonded(in_bonded, mapping):
    ...
    for b_list, b_params in in_bonded:
        new_b_list = map(mapping.get, b_list)
        if new_b_list.count(None) == len(new_b_list):
            continue
        else:
            new_b_list = map(lambda p: mapping.get(p, p), b_list)
...
def main():
    ...
    for l in mapping_file.readlines():
        t = l.split()
        (old_id, new_id), new_name = map(int, t[:2]), t[2]
    ...
    in_top.bonds.update(generate_new_bonded(in_top.bonds.items(), mapping))
    in_top.angles.update(generate_new_bonded(in_top.angles.items(), mapping))
    in_top.dihedrals.update(generate_new_bonded(in_top.dihedrals.items(), mapping))
    in_top.improper_dihedrals.update(generate_new_bonded(in_top.improper_dihedrals.items(), mapping))
    in_top.pairs.update(generate_new_bonded(in_top.pairs.items(), mapping))
