...
parser.add_argument('-i', '--interactive',
    help='Run interactive shell', action='store_true', default=False)
...
def get_color_nodes(gg, nodelist=None):
  if nodelist is None:
    nodelist = gg.nodes()
  return [
    color_node.get(gg.node[xx]['name'][0], 'yellow') for xx in nodelist
    ]
...
def draw_gg(gg, ommit=None):
  if ommit is None:
    ommit = []
  nodelist = [
      zz for zz, v in gg.node.iteritems()
      if v['name'] not in ommit
      ]
  nx.draw_graphviz(
      gg,
      with_labels=True,
      ...
      node_size=600,
      labels={xx: gg.node[xx]['name']
        for xx in gg.nodes() if gg.node[xx]['name'] not in ommit},
      )
  plt.show()
...
if args.draw:
  color_nodes = [
      color_node.get(g.node[x]['name'][0], 'yellow') for x in g.nodes()
      ]
...
# Compare with the topology itself.
  ...
  if not top_angles == angles:
    ...
    for idx, ang in enumerate((top_angles - angles)):
      print idx, map(top.atoms.get, ang)
...
# Check dihedrals
  ...
  if not top_dihedrals == dihedrals:
    ...
    for idx, dih in enumerate((top_dihedrals - dihedrals)):
      print idx, map(top.atoms.get, dih)
...
# Check if the charge is sum to 0
  total_charge = sum(x.charge if x.charge is not None else 0.0 for x in top.atoms.values())
