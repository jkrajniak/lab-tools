...
for at_id in sorted(top.atoms):
    ...
    fmt = "%s\t%s\t%s\t%s\t%s\t%s"
    data = [new_at_id, at.atom_type, at.chain_idx, at.chain_name, at.name, at.cgnr]
...
output_file.write("\n[ bonds ]\n")
new_bonds_dict = {tuple(map(replace_index.get, k)): v for k, v in top.bonds.items()}
for k in sorted(new_bonds_dict):
    output_file.write("%s\t%s\n" % ("\t".join(map(str, k)), " ".join(new_bonds_dict[k])))

output_file.write("\n[ angles ]\n")
new_dict = {tuple(map(replace_index.get, k)): v for k, v in top.angles.items()}
for k in sorted(new_dict):
    output_file.write("%s\t%s\n" % ("\t".join(map(str, k)), " ".join(new_dict[k])))

output_file.write("\n[ dihedrals ]\n")
new_dict = {tuple(map(replace_index.get, k)): v for k, v in top.dihedrals.items()}
for k in sorted(new_dict):
    output_file.write("%s\t%s\n" % ("\t".join(map(str, k)), " ".join(new_dict[k])))

output_file.write("\n[ dihedrals ]\n")
new_dict = {tuple(map(replace_index.get, k)): v for k, v in top.improper_dihedrals.items()}
for k in sorted(new_dict):
    output_file.write("%s\t%s\n" % ("\t".join(map(str, k)), " ".join(new_dict[k])))

output_file.write("\n[ pairs ]\n")
new_dict = {tuple(map(replace_index.get, k)): v for k, v in top.pairs.items()}
for k in sorted(new_dict):
    output_file.write("%s\t%s\n" % ("\t".join(map(str, k)), " ".join(new_dict[k])))
