...
def _args():
    ...
    parser.add_argument(
        "--mix_type", type=int, default=0, choices=[0, 1], help="coupling type, 0 for arithmetic, 1 for geometric"
    )
    parser.add_argument("--output_type", default="espp", choices=["espp", "gromacs"], help="Define the output format")
...
def mix_geometric(tab1, tab2, coupling, constant):
    ...
    out_tab[:, 1] = pow(e1 + constant, coupling) + pow(e2 + constant, (1 - coupling)) - constant
    out_tab[:, 2] = coupling * pow(e1 + constant, coupling - 1.0) * f1 + (1.0 - coupling) * pow(e2 + constant, -coupling) * f2
    return out_tab
...
def main():
    ...
    if args.mix_type == 0:
        ...
        print(("\tx={}\n\tU1={}\n\tU2={}\n\tC={}".format(args.scaling, args.tab1, args.tab2, args.constant)))
    ...
    np.savetxt(
        args.out_table,
        mixed_table,
        header="Mixed of {} and {} at {}".format(args.tab1, args.tab2, datetime.datetime.now()),
        fmt="%2.9e",
    )
