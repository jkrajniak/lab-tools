...
def read_settings(input_file):
    ...
    output_tuple = collections.namedtuple("Output", ["name_seq", "type2chain", "mol_name", "nrexcl", "nrmols"])
    return output_tuple(
        name_sequence, type2chain, molecule.attrib["name"], molecule.attrib["nrexcl"], molecule.attrib["nrmols"]
    )
...
def lammps2gromacs_ff(lmp_input, settings):
    ...
    if "improper_style" in lmp_input.force_field:
        ...
        if improper_style == "cvff":
            coeff = {k: list(map(float, v)) for k, v in lmp_input.force_field["improper"].items()}
            print(coeff)
            for k, v in list(coeff.items()):
                K = kcal2kJ(float(v[0]))
                int(v[1])
        ...
        if pair_style == "table":
            ...
            for pair_type, output_type in list(settings.typeid2name.items()):
                try:
                    ...
                    print(
                        (
                            ("Could not find mass value for type {} (GROMACS type: {})" " check your LAMMPS data file").format(
                                pair_type, output_type
                            )
                        )
                    )
                    raise ex
                try:
                    ...
                    print(
                        (
                            (
                                "Could not find charge value for type {} (GROMACS type: {})" " check your LAMMPS data file"
                            ).format(pair_type, output_type)
                        )
                    )
                    raise ex
...
def build_gromacs(lr, settings, output_file):
    ...
    output.moleculetype = [{"name": settings.mol_name, "nrexcl": settings.nrexcl}]
    output.molecules = [{"name": settings.mol_name, "mol": settings.nrmols}]
...
def _args():
    parser = argparse.ArgumentParser(description="Convert LAMMPS to GROMACS topology", add_help=True)
