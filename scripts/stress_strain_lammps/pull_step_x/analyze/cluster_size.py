...
for input_data in os.listdir('.'):
    if re_filename.match(input_data):
        ...
        print((input_data, lr.timestep, conversion, num_components, largest_size, avg_component_size, avg_deg_epo, avg_deg_hdd, avg_deg_jef, avg_deg_ipd))
        output_data.append([lr.timestep, conversion, num_components, largest_size, avg_component_size, avg_deg_epo, avg_deg_hdd, avg_deg_jef, avg_deg_ipd])

output_data.sort(key=lambda x: x[0])
np.savetxt('{}_cluster_data.csv'.format(args.sys_id), np.nan_to_num(output_data), header='timestep conversion num_components largest_size avg_component_size avg_deg_epo avg_deg_hdd avg_deg_jef avg_deg_ipd')
