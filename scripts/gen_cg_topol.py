...
def _args():
    ...
    parser.add_argument("--options", help="XML mapping files (comma separated) (VOTCA format)", required=True)
    parser.add_argument("--itp", help="Optional ITP file with bondtypes, angletypes etc.", required=False)
...
def map_topology(settings, in_top, itp_top):
    ...
    for at_id in sorted(in_top.atoms):
        ...
        if current_cg_bead is None:
            ...
            cg_bead = [cg_name, cg_type, cg_bead_mass, resid, at_data.chain_idx, at_data.chain_name]
        ...
        for bs in sorted(input_list):
            ...
            if res_ids.count(res_ids[0]) != len(res_ids):
                ...
                try:
                    ...
                    print(("BS: {} res_ids: {} rtypes: {}".format(bs, res_ids, rtypes)))
                    raise ex
