...
def _args():
    parser = argparse.ArgumentParser(description="Converts LAMMPS tabulated potentials to Espresso++ format")
    ...
    for line in input_f:
        ...
        if not l.startswith("#") and not l.startswith("N") and l:
            ...
            if len(sl) == 4:
                ...
                output_f.write(
                    "{} {} {}\n".format(float(sl[0]) * args.length_scale, float(sl[1]) * 4.184, float(sl[2]) * 41.84)
                )
    ...
    for line in input_f:
        ...
        if not l.startswith("#") and not l.startswith("N") and l:
            ...
            if len(sl) < 3:
                continue
            output_f.write(
                "{} {} {}\n".format(math.radians(float(sl[1])), float(sl[2]) * 4.184, float(sl[3]) * 4.184 * 180.0 / math.pi)
            )
    ...
    for line in input_f:
        ...
        if not l.startswith("#") and not l.startswith("N") and l:
            ...
            if nof:
                ...
                output_f.write("{} {} {}\n".format(phi, float(sl[2]) * 4.184, float(sl[3]) * 4.184 * 180.0 / math.pi))
        elif l.startswith("N"):
