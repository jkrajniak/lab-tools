...
def _args():
    ...
    parser.add_argument("--bonds", help="List of bonds, ex. 1-2,2-3,3-4,1-4", action=ListAction, default=[])
    parser.add_argument("--angles", help="List of angles, ex. 1-2-3,2-3-4", action=ListAction, default=[])
    parser.add_argument("--torsions", help="List of torsions, ex. 1-2-3-4", action=ListAction, default=[])
...
def save_data(name, array, definitions, filename_prefix, file_template, timeserie):
    for idx, definition in enumerate(definitions):
        ...
        if timeserie:
            np.save(
                filename, np.array([np.array([x for x in arr[idx :: len(definitions)] if x is not None]) for arr in array])
            )
        else:
...
def main_local():
    ...
    if is_gromacs:
        data = pmi.invoke("_gromacs_processing", args, q_bonds, q_angles, q_torsions, frame_range_list, pos_cons, box)
    else:
        data = pmi.invoke("_h5_processing", args, q_bonds, q_angles, q_torsions, frame_range_list, pos_cons, box)
